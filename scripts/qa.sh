...
FAILURES=0
WARNINGS=0
STALE_PATHS_REPORT="$PYTHON_CACHE_DIR/stale-paths.txt"
BUNDLE_SECRET_REPORT="$PYTHON_CACHE_DIR/bundle-secrets.txt"
...
if rg -n \
  ...
  "$ROOT" > "$STALE_PATHS_REPORT" 2>/dev/null; then
  fail "이동 전 경로 참조 발견: $STALE_PATHS_REPORT 확인"
else
  pass "이동 전 경로 참조 없음"
fi
  ...
  if [[ -x "$WEB_DIR/node_modules/.bin/next" ]] && \
    ...
    if [[ -d "$WEB_DIR/.next/static" ]] && \
      rg -I -l \
        ...
        "$WEB_DIR/.next/static" > "$BUNDLE_SECRET_REPORT" 2>/dev/null; then
