...
- [x] 공개 GitHub Actions의 암호화 백업 보관 승인 후 자동 작업 활성화
- [x] 변경 전 발송 메일의 좁은 범위 검색 승인 후 과거 중복 판별 보완
...
- 발송: 수신자 주소의 SHA-256 + 시장 + 접수번호로 전달 장부를 만든다. SMTP 접수 직후 DB 저장이 끊겨도 Gmail Sent에서 앱 전용 Message-ID만 검색해 확인한다. 확실하지 않으면 무조건 재발송하지 않는다. 재확인 작업이 동시에 실행돼도 새 발송 시도를 덮어쓰지 않는다. Gmail 이외 SMTP의 불명확한 결과는 확인 대기 상태로 유지한다.
  변경 전 메일은 사용자 승인 후 Gmail Sent에서 발신자·수신자·앱 브리핑 제목·접수번호를 함께 검색한다. SEC는 과거 원문 링크에 들어간 하이픈 없는 접수번호도 확인한다. [Gmail의 서버 측 검색](https://developers.google.com/workspace/gmail/imap/imap-extensions#extension_of_the_search_command_x-gm-raw)으로 일치 여부만 확인하며 제목·본문을 가져오는 FETCH는 사용하지 않는다. 한글은 UTF-8 검색 문자셋과 IMAP literal로 전송한다. 확인된 중복만 제외하고 새 공시만 발송하며, 전부 중복이면 SMTP를 호출하지 않는다. 검색 실패 시 재발송하지 않고 다음 실행에서 재확인한다. 삭제됐거나 검색되지 않는 이전 메일까지 발송 완료로 추정하지 않는다.
