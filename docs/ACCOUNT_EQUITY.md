...
종목이 없고 현금만 있는 계좌도 총자산 수신 기록이 있으면 선택 조건에 맞는 자산 이력에 표시된다.
원화 총계열의 `currency_breakdown`으로 KRW/USD/JPY 자산 상세를 지원한다. 독립 JPY 계열은 만들지 않으며 기존 일본 보유종목 기능은 변경하지 않는다.
...
각 관측값은 `date`, `valued_at`, `collected_at`, `calculated_at`, `equity`, `cash`, `stock_value`, `return_index`, `return_status`, `source`를 필수로 포함하며, 아래 `breakdown` 또는 `currency_breakdown`을 선택적으로 추가할 수 있다. 다른 임의 필드는 거부한다. 기존 v1 요청도 그대로 받는다.

### 원·달러·엔 상세 (`currency_breakdown`)

선택 통화 마이그레이션과 웹을 먼저 배포한다. `account-total-assets`/KRW에서 정확히 KRW/USD/JPY 3개 행을 받는다. 각 행은 `currency`, `cash`, `stock_value`, `cash_krw`, `stock_value_krw`만 가진다. 금액은 기존 정밀도의 소수 문자열이며 현금만 음수를 허용한다. KRW 원금액과 환산액은 같아야 하고, 현금·주식 환산 합계는 관측값의 현금·주식·총액과 각각 2원 이내로 일치해야 한다. API와 DB에서 모두 검사한다.

다른 통화는 제외한다. 키움 원·달러는 결제예정 현금, 엔화는 통화별 평가 API가 제공하는 원금액과 환산액을 사용한다. 한투는 통화별 잔고와 환율·별도 보유주식 조회를 대조하며, 모의 응답 불일치 때 기존 총자산만 보존한다. 통화별 금액을 총액에서 역산하지 않는다. 완전한 조회에서 부재가 확인된 통화만 0이다. 직접 등록 잔고를 추가하지 않는다.

기존 전체 통화 총액과 새 3개 통화 합계는 그래프 선이나 수익률로 연결하지 않는다. 독립 잔고 조회기는 실계좌 주문을 허용하지 않고 자산 조회·전송만 수행한다.
...
- 쿼리 없음: `{ series: [...], statuses: [...], statusUnavailable: false }`에 시리즈별 최신 관측값과 별도 수집 진단을 반환한다. 진단 조회만 실패하면 자산은 유지하고 `statusUnavailable: true`로 표시한다.
...
실제 투자(`/portfolio`)는 **원화 총자산(현금 포함) → 통화별 현금·주식 평가액**, 통화 상세가 없는 기존 총계열은 국내주식/미국주식/공통 현금을 표시한다. 모의매매(`/portfolio/paper`)는 **수신된 통합 총액·마지막 수집 시각**만 표시하며 통화 상세와 이력 그래프는 요청하지 않는다. 모의 수집 진단은 접힌 상세에서 확인한다. 시장별 현금을 임의 배분하지 않는다. 총계열이 없으면 '연동 총자산 기록 없음'을 표시하고 직접 등록만 있는 경우·개별 범위만 수신된 경우를 구분한다. 상세 누락은 '상세 미확인'이며 0으로 채우지 않는다. 진단도 마지막 확인값이며 실시간 수집 성공을 보장하지 않는다.

자산 그래프는 일별 마지막 수집값이며 종가가 아니다. 국내·미국 그래프는 원화 **주식 평가액 추이이지 수익률이 아니다**. 누적 수익률은 기존 검증된 총계열 기준만 표시한다. 날짜 공백, 상세 누락, 미확인 수익률, 계좌 묶음/기준일/산식 변경은 선으로 잇지 않는다. 기간 필터는 표시 기간만 바꾸며 수익률을 새로 0%로 만들지 않는다. 웹에서 다른 계열 금액을 더하거나 오늘 환율로 과거 자산을 재평가하지 않는다. 키움 기존 국내/해외 계열은 저장소와 API에 보존하지만 화면 선택 항목에서는 제외하며 총계열에 이어 붙이지 않는다.

실계좌·모의계좌는 상단 화면 전환으로 분리한다. 각 화면의 계좌 선택은 **전체 증권사 / 키움증권 / 한국투자증권**이다. 선택 조건에 맞는 총자산 계열은 각각 표시하며 서로 합산하지 않는다. 같은 증권사·실/모의 유형의 계열이 여러 개면 연결 묶음 번호로 구분한다. 선택은 등록 보유종목·비중·자동매매 성과에도 적용되며 모든 시장을 포함한다. 기존 보유종목에는 실제 계좌 UUID와 일반·ISA 구분이 없으므로 같은 증권사의 여러 실계좌나 ISA를 임의 분류하지 않는다. 직접 등록 금액을 연동 총자산에 더하지 않는다. 화면 분리는 수집·저장·동기화 계약을 바꾸지 않는다.
...
## PUT /api/sync/account-status

[진단 마이그레이션](../supabase/migrations/20260918012641_account_collection_status.sql) 적용 후 사용한다.
기존 연동 Bearer 토큰으로 `{version:1,statuses:[...]}`를 보낸다. 최대 20개이며 각 항목은 정확히
`account_ref`(무작위 UUID), `broker`(KIWOOM/KIS), `account_type`(paper/live), `checked_at`(실제 확인 ISO 시각), `code`만 갖는다.
코드는 `total_verified`, `other_currency_assets`, `total_unverified`, `collection_failed` 중 하나다.
계좌번호·키 지문·금액·원문 오류는 받지 않는다. 회원은 토큰으로 결정하며 자신의 진단만 조회할 수 있다.
응답은 `{ok:true,synced:변경_건수}`. 이전 시각과 동일 내용 재전송은 최신 상태를 덮지 않고 빈 배열도 삭제하지 않는다.
[일관성 마이그레이션](../supabase/migrations/20260918015154_account_status_consistency.sql)은 같은 계좌·시각의 서로 다른 코드를 409로, 요청 내 중복 계좌를 400으로 거부한다. 충돌 시 요청 전체를 취소해 일부 진단만 저장되지 않게 한다.
진단과 자산 조회는 병렬이며 진단 조회에는 별도 3초 제한을 둔다. 진단 전송·오류 알림 실패가 자산 전송을 막지 않는다.
`total_verified`는 송신 측 검증 결과다. 같은 계좌·같거나 더 최신 시각의 총자산을 웹이 받은 경우만 '총자산 수신 완료'로 표시하며, 진단보다 최신 총자산이 있으면 과거 실패와 구분한다.
이 저장은 보유종목·계좌 금액·주문과 독립적이다. Stock-Trading 실행기가 새 버전을 읽어야 자동 진단 전송이 시작된다.
