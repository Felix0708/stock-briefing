...
두 증권사 공통 화면은 **원화 총자산(현금 포함) → 국내주식/미국주식 평가액(현금 제외)/공통 현금**이다. 시장별 현금을 임의 배분하지 않는다. 총계열이 없으면 '연동 총자산 기록 없음'을 표시하고 직접 등록만 있는 경우·개별 범위만 수신된 경우를 구분한다. 상세 누락은 '상세 미확인'이며 0으로 채우지 않는다. 수집 진단이 수신되면 확인 시각과 정해진 보류 사유를 함께 표시한다. 진단도 마지막 확인값이며 실시간 수집 성공을 보장하지 않는다.
...
계좌 선택은 **전체 계좌 / 전체 실계좌 / 전체 모의계좌 / 키움증권 · 실/모의 전체 / 한국투자증권 · 실/모의 전체**의 5개만 제공한다. 선택 조건에 맞는 총자산 계열은 각각 표시하며 서로 합산하지 않는다. 같은 증권사·실/모의 유형의 계열이 여러 개면 연결 묶음 번호로 구분한다. 선택은 등록 보유종목·비중·자동매매 성과의 증권사 및 실/모의 필터에도 적용되며 모든 시장을 포함한다. 기존 보유종목에는 실제 계좌 UUID가 없으므로 같은 증권사의 여러 실계좌까지 구분할 수는 없다. 직접 등록 금액을 연동 총자산에 더하지 않는다.
