...
test("all cookie mutation routes, including future API routes, require an exact Origin", async () => {
  ...
  for (const path of routes) {
    ...
    if (["/api/ask", "/api/sync/holdings", "/api/sync/account-equity", "/api/sync/account-status", "/api/sync/broker-holdings", "/api/sync/tax-estimate"].includes(url)) continue;
  ...
  for (const [path, method] of [["/api/auth/me", "GET"], ["/api/ask", "POST"], ["/api/sync/holdings", "PUT"], ["/api/sync/account-equity", "PUT"], ["/api/sync/account-status", "PUT"], ["/api/sync/broker-holdings", "PUT"], ["/api/sync/tax-estimate", "PUT"]]) {
