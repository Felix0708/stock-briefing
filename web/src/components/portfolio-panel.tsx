...
export function PortfolioPanel() {
  ...
  const [equityState, setEquityState] = useState<{owner: string; series: EquityRecord[]; statuses: AccountStatus[]; statusUnavailable: boolean; error: boolean}>({owner:"",series:[],statuses:[],statusUnavailable:false,error:false});
  ...
  const loadQuotes = useCallback(async (rows: Holding[]) => {
    try {
      ...
      setQuotesError("시세 갱신 실패 · 이전 조회값이 있다면 유지됩니다. 다시 새로고침해 주세요.");
    }
  }, []);
  ...
  useEffect(() => {
    ...
    setEquityBusy(true);
    void api<{series: EquityRecord[]; statuses?: AccountStatus[]; statusUnavailable?: boolean}>("/api/account-equity", {signal:controller.signal,cache:"no-store"})
      .then(data => { if (!Array.isArray(data.series)) throw new Error("Invalid account list");
        if (!controller.signal.aborted) setEquityState({owner:memberEmail,series:data.series,statuses:data.statuses ?? [],statusUnavailable:data.statusUnavailable ?? false,error:false}); })
      .catch(() => { if (!controller.signal.aborted) setEquityState(current => ({owner:memberEmail,series:current.owner === memberEmail ? current.series : [],statuses:current.owner === memberEmail ? current.statuses : [],statusUnavailable:true,error:true})); })
      .finally(() => { if (!controller.signal.aborted) setEquityBusy(false); });
    return () => controller.abort();
  }, [memberEmail, equityRefresh]);

  useEffect(() => {
    if (!user || (holdings.length === 0 && performance.length === 0)) return;
  ...
  }, [user, holdings.length, performance.length, quotesAsOf, loadHoldings]);

  useEffect(() => {
    try {
      const saved = JSON.parse(localStorage.getItem("portfolio-display") ?? "{}");
      setShowPricesInKrw(saved.krw === true);
      setShowPerformanceInKrw(saved.performanceKrw === true);
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem("portfolio-display", JSON.stringify({ krw: showPricesInKrw, performanceKrw: showPerformanceInKrw }));
    } catch { /* Display preferences do not block the portfolio. */ }
  }, [preferencesLoaded, showPricesInKrw, showPerformanceInKrw]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      void (async () => {
        try {
          await api(`/api/holdings?code=${encodeURIComponent(pendingDelete.stock_code)}&market=${pendingDelete.market}&broker=${encodeURIComponent(pendingDelete.broker)}&quantity=${pendingDelete.quantity}&avg_price=${pendingDelete.avg_price}`, { method: "DELETE" });
          await loadHoldings();
        } catch {
          setListError("삭제 결과를 확인하지 못했거나 잔고가 변경되었습니다. 새로고침 후 확인해 주세요.");
        } finally {
  ...
  const computed = (() => {
    ...
    const weightBase = rows.map((row) => row.valueKrw ?? row.costKrw);
    const accountTotals = new Map<string, number>();
    rows.forEach((row, index) => {
      const key = accountGroupKey(row.holding);
      accountTotals.set(key, (accountTotals.get(key) ?? 0) + (weightBase[index] ?? 0));
    });
    const weights = rows.map((row, index) => {
      const total = accountTotals.get(accountGroupKey(row.holding)) ?? 0;
      const incomplete = rows.some((item, i) => accountGroupKey(item.holding) === accountGroupKey(row.holding) && weightBase[i] === null);
      return !incomplete && total > 0 ? ((weightBase[index] ?? 0) / total) * 100 : null;
    });
    ...
    const charts = holdingGroups.map(({ key, rows: group }) => {
        ...
        return {
          ...
          note: !complete ? "환율 정보가 없어 비중을 계산할 수 없습니다." : group.some((row) => row.valueKrw === null) ? "시세 없는 종목은 현재 환율로 환산한 매입가를 사용한 추정 비중입니다." : "현재 평가금액 기준 비중",
          slices: !complete ? [] : group.map((row, index) => ({
    ...
    return { rows, holdingGroups, summaries, weights, charts };
  })();
    ...
    return (
      <section className="pf-card" aria-labelledby="pf-auth-title">
        ...
        <p className="pf-muted">
          보유 종목을 등록하면 마지막 조회 시세 기준 수익률과 비중을 볼 수 있습니다.
        </p>
  ...
  return (
    <div className="pf-stack">
      ...
      <details className="pf-card pf-fold" open={integrationOpen || !!plainToken} onToggle={event => setIntegrationOpen(event.currentTarget.open)}>
        <summary><h2 id="pf-integration-title">Stock-Trading 자동 연동</h2><span className="pf-muted">{tokenStatus.active ? "토큰 활성 · 설정 펼치기" : "연동 설정 펼치기"}</span></summary>
        <p className="pf-muted">
          로컬 Stock-Trading이 보유종목·매매 성과·계좌 자산 이력을 동기화합니다. 이 웹에서 증권 주문을 보내지 않습니다.
      ...
      <section className="pf-card" aria-labelledby="pf-equity-title">
        ...
        {!equityBusy && !equityState.error && missingTotals.map(group => {
          const partials = equitySeries.filter(row => row.broker === group.broker && row.account_type === group.account_type && row.scope !== "account-total-assets");
      ...
      <section className="pf-card" aria-labelledby="pf-list-title">
        <div className="pf-list-head">
          <h2 id="pf-list-title">등록 보유종목 평가</h2>
          <div className="pf-list-tools">
            ...
            {quotesAsOf && (
              <span className="pf-muted pf-asof">
                마지막 조회 {new Date(quotesAsOf).toLocaleTimeString("ko-KR")}
        ...
        {computed.rows.length === 0 && !listBusy ? (
          ...
          <>
            ...
            <h3>{summary.type === "paper" ? "모의계좌" : "실계좌"} 등록 주식 합계</h3>
            <div className="pf-summary">
              ...
              <div>
                ...
                <strong className={plClass(summary.totalPl)}>
                  {summary.hasQuotes
                      ...
                      <td className="pf-table-action">
                        {row.holding.source === "manual" && (
                          <button
                            ...
                            onClick={() => setPendingDelete(row.holding)}
                          >
                            ✕
                          </button>
                        )}
            ...
            <div className="pf-pie-grid">
              {computed.charts.map((chart) => (
                <PieChart key={chart.key} title={chart.title} slices={chart.slices} note={chart.note} />
              ))}
            </div>
          </>
        )}
        ...
        <div className="pf-performance" aria-labelledby="pf-performance-title">
          ...
          {visiblePerformance.length === 0 ? (
            <p className="pf-muted">선택한 조건의 자동매매 성과가 없습니다.</p>
          ) : (
...
function PieChart({ title, slices, note }: { title: string; slices: PieSlice[]; note: string }) {
  const radius = 42;
  ...
  return (
    <article className="pf-pie-card">
      <h3>{title}</h3>
      <p className="pf-muted">{note}</p>
      {slices.length > 0 && <div className="pf-pie" role="img" aria-label={`${title} 종목별 비중 원 그래프`}>
        ...
        <ul className="pf-legend">
          {slices.map((slice) => (
            <li key={slice.key}>
              ...
              <span className="pf-legend-label">{slice.label}</span>
