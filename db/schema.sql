...
-- public Data API 스키마 밖에 pgvector 확장을 둔다.
...
create table if not exists filings (
  ...
  embedding   extensions.vector(768), -- Gemini 임베딩 (768차원)
...
-- 서버 전용 테이블임을 명시한다. 권한 revoke와 함께 이중으로 차단한다.
...
create index if not exists filings_embedding_idx
  on filings using hnsw (embedding extensions.vector_cosine_ops);

-- 이전 3개 인자 버전이 남아 PUBLIC으로 호출되는 것을 막는다.
drop function if exists public.match_filings(extensions.vector, integer, text);
...
revoke execute on function public.match_filings(extensions.vector, integer, text, float) from public;
