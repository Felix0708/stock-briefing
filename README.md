...
계좌 총자산·현금·날짜별 이력은 별도의 [계좌 자산 API v1](docs/ACCOUNT_EQUITY.md)을 사용합니다. 두 증권사 모두 원화 총자산 아래 검증된 국내·미국 주식 평가액과 공통 현금을 표시하며, 미확인 상세는 0으로 채우지 않습니다. 기존 분리 이력은 보존합니다. 아래 기존 보유종목 API 계약은 그대로 유지합니다.
