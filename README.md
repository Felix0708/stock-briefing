...
상단의 실제 투자(`/portfolio`, 기본)·모의매매(`/portfolio/paper`)에서 화면을 전환합니다. 각 화면의 계좌 선택은 전체 증권사·키움·한투 3개이며 자산·보유종목·비중·자동매매 성과에 함께 적용합니다. 실제 투자에는 현금·통화별 상세와 자산 이력을, 모의매매에는 수신된 증권사별 통합 총액과 수집 시각만 표시합니다. 모의 수집 진단은 접어서 보관하며 데이터·수집·동기화 계약을 삭제하거나 변경하지 않습니다. 서로 다른 연결 계좌 묶음은 임의로 합산하지 않습니다.
현금 포함 연동 총자산과 등록 주식 평가는 수집 시각·환율·범위가 다릅니다. 예상 세금·직접 매매 이력은 증권사 필터와 무관한 모든 실계좌 기준이며 실제 투자 화면에서만 제공합니다. 브리핑 상태·백업은 모의를 포함한 전체 포트폴리오 기준입니다. 일반·ISA 구분 정보는 아직 연동 계약에 없어 계좌 종류를 추정하지 않습니다.
