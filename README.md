...
[웹 축 — Next.js on Vercel]
  ...
  /portfolio   실제 투자: 실계좌 자산 · 직접/자동 보유종목 · 매매 이력 · 예상 세금
  /portfolio/paper 모의매매: 증권사별 통합 총자산 · 모의 보유종목 · 자동매매 성과
...
아래 종목·수량·금액·성과는 API 형식을 설명하는 가상 예시이며 운영자의 계좌나 투자 설정이 아닙니다. 이용자는 자신의 증권사·계정유형과 실제 잔고에 맞게 구성합니다. 매매 지표나 전략은 이 서비스가 지정하지 않습니다.

계좌 총자산·현금·날짜별 이력은 별도의 [계좌 자산 API v1](docs/ACCOUNT_EQUITY.md)을 사용합니다. 원·달러·엔의 통화별 현금·주식 평가액과 원화 환산 합계를 지원하며, 미확인 상세는 0으로 채우지 않습니다. 통화별 대조가 안 된 계좌는 검증된 기존 총액·상세만 표시합니다. 기존 분리 이력과 아래 보유종목 API 계약은 보존합니다.
키움의 기존 국내(KRW)·미국(USD) 이력은 별도로 보존하고, 송신 측에서 검증한 원화 총자산은 새 계열로 시작합니다. 한투 계좌 전체자산(KRW)은 국내·해외를 포함하므로 국내 자산을 다시 더하지 않습니다.
상단의 실제 투자(`/portfolio`, 기본)·모의매매(`/portfolio/paper`)에서 화면을 전환합니다. 실제 투자에는 전체·키움·한투와 키움 ISA·한투 ISA 선택이 있으며, 모의는 기존 3개 메뉴를 유지합니다. 선택은 자산·보유종목·비중·자동매매 성과에 함께 적용합니다. 실제 투자에는 현금·통화별 상세와 자산 이력을, 모의매매에는 수신된 증권사별 통합 총액과 수집 시각만 표시합니다. 서로 다른 연결 계좌 묶음은 임의로 합산하지 않습니다.
현금 포함 연동 총자산과 등록 주식 평가는 수집 시각·환율·범위가 다릅니다. 예상 세금·직접 매매 이력은 증권사 필터와 무관한 실계좌 기준이며 실제 투자 화면에서만 제공합니다. ISA는 국내 상장 종목만 등록하므로 일반 해외주식 양도세에는 포함하지 않습니다. 브리핑 상태·백업은 모의를 포함한 전체 포트폴리오 기준입니다.
수집 진단은 `PUT /api/sync/account-status`로 별도 수신합니다. [진단 테이블 마이그레이션](supabase/migrations/20260918012641_account_collection_status.sql)을 먼저 적용해야 합니다. 계좌번호·잔액·원문 오류 대신 정해진 상태 코드만 저장하며 보유종목·금액 이력은 변경하지 않습니다. USD 외 자산이 있으면 누락 합산하지 않고 보류 사유를 표시합니다. 입출금 조정 수익률은 해당 계좌·기간의 입출금 증빙이 필요하며, 자산 관측만 늘어도 검증되지는 않습니다.
...
- 등록 주식 합계에는 원화 환산액과 통화별 원금액(KRW/USD/JPY)을 함께 표시한다. 환율이 없더라도 외화 원금액은 유지하며, 일부 시세만 있으면 일부 합계임을 표시한다.
- [미국·일본 예상 세금](docs/PORTFOLIO_TAX.md)은 **실계좌의 기록된 연간 매도손익을 자동 집계**한다. 직접 매매 이력과 Stock-Trading의 실계좌 매도 집계(부분 매도 포함)를 합치고, 2026년 일반 과세·공제 250만 원 합산 1회를 가정한다. 기본 결과는 현재 환율·비용 전 참고 추정으로 세무 신고용이 아니다. 모의·국내주식·배당·특례는 제외한다. 선택적 직접 보정값은 저장하지 않는다.
...
포트폴리오에서 잔고·매매 CSV와 JSON 백업을 내려받을 수 있다. JSON 복원은 미리보기·교체 확인 후 실행하며 복원 직전 상태를 따로 보관한다. 자동 암호화 백업은 배포자가 보관 위치·주기·공개 범위를 검토한 뒤 설정한다. 제공된 워크플로는 GitHub Actions에 암호문만 30일 보관하며 격리 PostgreSQL에서 복원 대조한다. 범위와 복구 절차는 [복구 운영 문서](docs/RECOVERY_WORK.md)를 참고한다.
...
관심 종목, 수신 주소, 모델과 호출 예산은 배포자가 선택한다. 예시 파일은 개인 운영 설정을 복제하는 용도가 아니다. `watchlist.yaml`은 공개 수집 목록이므로 비공개 보유종목은 로그인 후 포트폴리오에 등록하고 공개 동의 여부를 직접 선택한다. 정기 실행 시간은 제공된 워크플로의 기본값이며 배포 환경에 맞게 조정할 수 있다.
...
`20260918055337_live_holdings_ownership.sql` 적용 후 배포합니다. `PUT /api/sync/broker-holdings`는 같은 연동 토큰으로 `{version:1,snapshots:[{broker,account_kind,market,collected_at,holdings}]}`를 받습니다. `account_kind`는 `general|isa`, `market`은 `KR|US`이며 ISA는 KR만 허용합니다. 종목은 `{stock_code,stock_name,quantity,avg_price,automated_quantity}`입니다. `automated_quantity`는 검증된 자동매매 보유량 또는 미확인 `null`이며 실제 수량보다 클 수 없습니다. 잔고·수량·주문 기록만으로 매수 주체별 평단가를 만들지 않습니다.

- 최신 완전 조회에 성공한 계좌·시장의 잔고를 우선 표시합니다. 성공한 빈 잔고는 0종목, 조회 실패는 이전 기록 유지입니다. 모의 및 다른 계좌는 변경하지 않습니다.
- 수기 입력 원본·매매 이력은 삭제하지 않으며, 같은 조회 범위의 수기 종목은 중복 합산하지 않습니다. 조회 잔고는 수기 수정·삭제 버튼이 없습니다.
- 직접투자와 자동매매가 같은 종목을 보유하면 한 행에서 각각의 수량을 표시합니다. 주문 이력 부재·미확정 주문·수량 충돌은 ‘매수 주체 확인 필요’입니다.
- ISA 수기 등록은 기존 계좌별 장부의 별도 키 `KIS_ISA|KIWOOM_ISA`를 사용하므로 일반계좌의 같은 종목과 매매·정정·백업 복원까지 분리됩니다. 연결된 ISA는 직접 재등록할 필요가 없습니다.
- 브리핑 수집도 RLS가 적용되는 `briefing_holdings` 뷰의 동일한 유효 잔고를 사용합니다. 증권사 최신 조회 잔고는 복원으로 덮어쓰지 않으며 수기 장부만 기존 백업에 포함됩니다.
- `ip_not_registered`는 키움 8050 전용 진단입니다. 등록된 네트워크로 복귀하면 다음 정기 수집에 재시도하며, 기존 정상 금액을 지우지 않습니다.
