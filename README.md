...
- 기존 ‘종목 등록’은 잔고 보정용이다. ‘직접 투자 · 매매 이력’은 현재 잔고에서 입력 순서대로 매수·매도를 적용한다. 기존 잔고의 과거 거래는 생성하지 않고, 과거 날짜를 입력해도 이전 거래를 소급 재계산하지 않는다.
- 거래 이력은 변경 불가 기록이다. 같은 요청 ID 재전송은 중복 처리하지 않고, 잔고 부족 매도는 거부한다. 잔고 삭제는 매도 처리하지 않으며 8초 안에 취소할 수 있다.
