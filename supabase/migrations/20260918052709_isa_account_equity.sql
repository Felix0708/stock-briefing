-- Optional ISA metadata and reconciled domestic holdings; same ownership and grants.
begin;
...
declare
  ...
  detail jsonb;
  currency_row jsonb;
  currency_cash numeric;
  currency_stock numeric;
...
begin
  ...
  for item in select value from jsonb_array_elements(records) loop
    if jsonb_typeof(item) <> 'object' or not (item ?& required_keys)
      or exists (select 1 from jsonb_object_keys(item) k where k <> all(required_keys || array['account_group_ref','breakdown','currency_breakdown','account_kind','isa_holdings']))
    ...
    if item ? 'currency_breakdown' then
      detail := item->'currency_breakdown';
      if jsonb_typeof(detail) is distinct from 'array' then
        raise sqlstate 'PT400' using message = 'invalid currencies';
      end if;
      if jsonb_array_length(detail) <> 3 or item->>'scope' is distinct from 'account-total-assets'
        or item->>'currency' is distinct from 'KRW'
        or item->'equity' = 'null'::jsonb or item->'cash' = 'null'::jsonb or item->'stock_value' = 'null'::jsonb
        or (select count(distinct r->>'currency') from jsonb_array_elements(detail) r) <> 3 then
        raise sqlstate 'PT400' using message = 'invalid currency scope';
      end if;
      currency_cash := 0; currency_stock := 0;
      for currency_row in select value from jsonb_array_elements(detail) loop
        if jsonb_typeof(currency_row) is distinct from 'object'
          or not (currency_row ?& array['currency','cash','stock_value','cash_krw','stock_value_krw'])
          or (select count(*) from jsonb_object_keys(currency_row)) <> 5
          or not coalesce(currency_row->>'currency' in ('KRW','USD','JPY'),false) then
          raise sqlstate 'PT400' using message = 'invalid currency row';
        end if;
        foreach field in array array['cash','stock_value','cash_krw','stock_value_krw'] loop
          if jsonb_typeof(currency_row->field) is distinct from 'string'
            or currency_row->>field !~ '^-?(0|[1-9][0-9]{0,15})(\.[0-9]{1,8})?$'
            or (field in ('stock_value','stock_value_krw') and left(currency_row->>field,1) = '-') then
            raise sqlstate 'PT400' using message = 'invalid currency amount';
          end if;
        end loop;
        if jsonb_typeof(currency_row->'currency') is distinct from 'string'
          or (currency_row->>'currency' = 'KRW' and (
            (currency_row->>'cash')::numeric <> (currency_row->>'cash_krw')::numeric
            or (currency_row->>'stock_value')::numeric <> (currency_row->>'stock_value_krw')::numeric)) then
          raise sqlstate 'PT400' using message = 'invalid KRW conversion';
        end if;
        currency_cash := currency_cash + (currency_row->>'cash_krw')::numeric;
        currency_stock := currency_stock + (currency_row->>'stock_value_krw')::numeric;
      end loop;
      if abs(currency_cash - (item->>'cash')::numeric) > 2
        or abs(currency_stock - (item->>'stock_value')::numeric) > 2
        or abs(currency_cash + currency_stock - (item->>'equity')::numeric) > 2 then
        raise sqlstate 'PT400' using message = 'currency total mismatch';
      end if;
    end if;


    if item ? 'account_kind' then
      if item->>'account_kind' is distinct from 'isa' or item->>'account_type' is distinct from 'live'
        or item->>'currency' is distinct from 'KRW' or item->>'scope' is distinct from 'account-total-assets'
        or item ? 'breakdown' or item ? 'currency_breakdown'
        or jsonb_typeof(item->'isa_holdings') is distinct from 'array'
        or item->'equity' = 'null'::jsonb or item->'cash' = 'null'::jsonb or item->'stock_value' = 'null'::jsonb
      then raise sqlstate 'PT400' using message='invalid ISA scope'; end if;
      detail := item->'isa_holdings';
      if jsonb_array_length(detail)>200 or (select count(distinct r->>'code') from jsonb_array_elements(detail) r)<>jsonb_array_length(detail)
      then raise sqlstate 'PT400' using message='invalid ISA holdings'; end if;
      currency_stock := 0;
      for currency_row in select value from jsonb_array_elements(detail) loop
        if jsonb_typeof(currency_row) is distinct from 'object' then raise sqlstate 'PT400' using message='invalid ISA holding'; end if;
        if not (currency_row ?& array['code','name','quantity','value']) or (select count(*) from jsonb_object_keys(currency_row))<>4
          or exists(select 1 from jsonb_each(currency_row) v where jsonb_typeof(v.value)<>'string')
          or currency_row->>'code' !~ '^\d{6}$' or length(btrim(currency_row->>'name'))=0 or length(currency_row->>'name')>100
          or currency_row->>'quantity' !~ '^[1-9][0-9]{0,14}$'
          or currency_row->>'value' !~ '^(0|[1-9][0-9]{0,15})(\.[0-9]{1,8})?$'
        then raise sqlstate 'PT400' using message='invalid ISA holding'; end if;
        currency_stock := currency_stock + (currency_row->>'value')::numeric;
      end loop;
      if abs(currency_stock-(item->>'stock_value')::numeric)>2
        or abs(currency_stock+(item->>'cash')::numeric-(item->>'equity')::numeric)>2
      then raise sqlstate 'PT400' using message='ISA reconciliation failed'; end if;
    elsif item ? 'isa_holdings' then raise sqlstate 'PT400' using message='ISA kind required';
    end if;
