-- Add KRW/USD/JPY-only subtotals; existing payloads, ownership and grants remain unchanged.
begin;
...
begin
  ...
  for item in select value from jsonb_array_elements(records) loop
    if jsonb_typeof(item) <> 'object' or not (item ?& required_keys)
      or exists (select 1 from jsonb_object_keys(item) k where k <> all(required_keys || array['account_group_ref','breakdown','currency_breakdown']))
